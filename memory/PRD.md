# Monster Huddle - Product Requirements Document
(formerly "EduQuiz" — rebranded 2026-06-01)
...
- **2026-06-01**: **Rebrand — "EduQuiz" → "Monster Huddle"**. New mascot logo (blue scholar-monster) shipped to `/frontend/public/monster-huddle-logo.png`. All brand mentions updated in `Login.js`, `Register.js`, `Dashboard.js` header, `LanguageContext.js` (EN + 中文), `index.html` `<title>`, FastAPI `title`, seed scripts and DB rows (`welcome_messages`, `notices`). Dashboard now features a large mascot panel on the **left** that spans vertically across "Welcome back, {user}!" and the "Current Level" row, with a tagline ("Learn. Level up. Together." / "一起学习,一起成长"). Layout is responsive — on mobile, the mascot stacks above the welcome text.
- **2026-06-17**: **Refactor (P1) — modular backend + componentized Manage Questions.** Backend `server.py` shrunk **1143 → 38 lines (-97%)**: extracted into `core.py` (db client, JWT, security deps, log_activity), `models.py` (all Pydantic), and `routes/{auth,users,subjects,quiz,leaderboard,admin}.py` (each owning its own `APIRouter`), composed via `routes/__init__.get_api_router()`. LIVE Competition module stays in `live_competition.py` (its own self-contained router). Frontend `ManageQuestions.js` shrunk **1074 → 596 lines (-44%)**: Add/Edit modal extracted to `manage-questions/QuestionForm.js` (owns its own save logic + media uploads), CSV export extracted to `manage-questions/csvUtils.js`, and media compress/read helpers to `manage-questions/mediaUtils.js`. Reset admin/demo passwords back to documented `admin123`/`demo123` after they had drifted between sessions. Verified end-to-end via curl smoke (auth, profile, levels, leaderboard, notices, admin questions/users/reports, bulk-delete, live tournaments) and admin UI screenshot test (Add Question modal with only EN A/B fills → "Question created" toast, list 50→51 then cleaned up). No console errors.
- **2026-06-17**: **Mobile support — PWA + Capacitor scaffolding.** Generated 8 icon variants from the Monster Huddle mascot (192/384/512 standard + 192/512 maskable + 180 apple-touch + 32/64 favicons). New `public/manifest.json` (standalone display, theme `#7c3aed`, 2 install-time shortcuts: "Continue learning" → `/`, "LIVE Competition" → `/live`). New `public/sw.js` (network-first; never caches `/api/*` or `/ws/*`; cache version bumped on each release). `index.html` updated with the manifest link, iOS meta tags, viewport-fit=cover, and a guarded service-worker registration that skips the Emergent visual-editor iframe. **Capacitor 7** installed (`@capacitor/core`, `cli`, `android`, `ios`) with `capacitor.config.json` (`appId: com.monsterhuddle.app`, `appName: Monster Huddle`, `webDir: build`). Full build instructions in `/app/MOBILE.md` covering both PWA install flows (Chrome Add-to-Home / iOS Share) and the native build pipeline (`yarn build && npx cap add android|ios && npx cap sync && npx cap open …`). Verified live: manifest 200, sw.js 200, all icons 200; head tags confirmed via DOM.
- **2026-06-17**: **PWA install banner — student dashboard nudge**. New `hooks/usePwaInstall.js` listens for `beforeinstallprompt` (Android/Chrome) + detects iOS (where the prompt API doesn't exist), tracks `display-mode: standalone`, and persists dismissal in `localStorage` for 30 days. New `components/InstallPwaBanner.js` renders a bilingual gradient banner ("Get the Monster Huddle app · Install" / "添加到主屏幕") on the Dashboard for **students only** (admins never see it) **only when** the app isn't already installed and the user hasn't dismissed it. Android users see an "Install" button that calls the native `prompt()`; iOS users see a "Show me how" expandable hint with the 3 Share → Add to Home Screen steps. Verified end-to-end via Playwright: admin → hidden, student + dispatched BIP → shown, click Install → `prompt()` invoked, click × → banner gone + dismissal saved. No console errors.
- **2026-06-17**: **PWA install banner — login page**. Same `<InstallPwaBanner>` component dropped between the "Monster Huddle" logo block and the login card in `Login.js`, so first-time visitors on phones see the install nudge **before** signing in. Banner reuses the shared `usePwaInstall` hook, so a single dismissal applies everywhere (30-day snooze in `localStorage`). Verified end-to-end via Playwright at 414×900 phone viewport: hidden by default, appears when `beforeinstallprompt` fires, Install button invokes native prompt, dismissal persists, login flow continues uninterrupted to `/dashboard`. No console errors.

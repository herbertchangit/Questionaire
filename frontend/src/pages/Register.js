...
import { Mail, Lock, User, UserPlus, Globe, Award } from 'lucide-react';
...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || (language === 'zh' ? '注册失败' : 'Registration failed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-500 via-purple-500 to-pink-500 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl">
          <h2 className="text-2xl font-black text-zinc-900 mb-2">{t('register_title')}</h2>
          <p className="text-zinc-500 mb-6">{t('register_subtitle')}</p>
